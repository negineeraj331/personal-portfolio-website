...
const typeConfig = {
  education: { icon: GraduationCap, label: "Education", gradient: "from-blue-500 to-cyan-500" },
  training: { icon: BookOpen, label: "Training", gradient: "from-violet-500 to-purple-500" },
  work: { icon: Briefcase, label: "Work", gradient: "from-green-500 to-emerald-500" },
};
      ...
      <div className="max-w-4xl mx-auto space-y-14">
        ...
        <div className="relative">
          <div className={`absolute left-6 top-0 bottom-0 w-px ${
            isDark
              ? "bg-gradient-to-b from-green-500/50 via-white/10 to-transparent"
              : "bg-gradient-to-b from-blue-500/50 via-gray-200 to-transparent"
          }`} />
                  ...
                  <motion.div whileHover={{ x: 6 }}
                    ...
                    <div className="flex flex-wrap items-start justify-between gap-2 mb-3">
                      <div>
                        ...
                        <p className={`text-sm font-medium ${isDark ? "text-green-400" : "text-blue-600"}`}>{exp.company}</p>
                      </div>
                    ...
                    <ul className="space-y-1.5">
                      {exp.description.map((item, j) => (
                        <li key={j} className={`text-sm leading-relaxed flex items-start gap-2 ${isDark ? "text-white/60" : "text-gray-600"}`}>
                          <span className={`mt-1.5 w-1.5 h-1.5 rounded-full flex-shrink-0 ${isDark ? "bg-green-400/60" : "bg-blue-400"}`} />
                          {item}
                        </li>
                      ))}
                    </ul>
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            {[
              ...
              <div key={stat.label}>
                <div className={`text-3xl font-black ${isDark ? "text-green-400" : "text-blue-600"}`}>{stat.value}</div>
